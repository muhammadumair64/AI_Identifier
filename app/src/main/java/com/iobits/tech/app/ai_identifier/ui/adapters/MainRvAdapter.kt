...
class MainRvAdapter(
...
) : RecyclerView.Adapter<MainRvAdapter.ModRvViewHolder>() {

    class ModRvViewHolder(val binding: ItemMainRvBinding) : RecyclerView.ViewHolder(
        binding.root
    )
    ...
    override fun onBindViewHolder(holder: ModRvViewHolder, position: Int) {
        ...
        holder.binding.image.setImageDrawable(ContextCompat.getDrawable(context, item.imgDrawableId))
        ...
        holder.binding.mainView.backgroundTintList = ContextCompat.getColorStateList(context, item.bgColorId)
