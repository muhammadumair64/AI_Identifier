...
@Singleton
class BillingManagerV5 @Inject constructor(@ApplicationContext private val context: Context) {
    //    private static volatile BillingManagerV5 instance;
    private var retryCount = 0
    private var billingClient: BillingClient? = null
    var productsAvailable: List<ProductDetails> = ArrayList()
...
//                        handlePurchaseHistory(p0, p1)
    ...
    private fun verifySubPurchase(purchases: Purchase) {
        ...
        if (purchases.products[0].equals(Constants.ITEM_SKU_PRO_USER_SUB_TRIAL)) {
            makeAppPremium()
        }

        if (purchases.products[0].equals("item_1"))
            thanksToast()
        if (purchases.products[0].equals("item_2"))
            thanksToast()
        if (purchases.products[0].equals("item_3")){
            makeAppAdsFree()
            thanksToast()
        }

        if (purchases.products[0].equals("item_4")){
            makeAppAdsFree()
            thanksToast()
        }
...
//                preferencesManager!!.put(PreferencesManager.Key.IS_APP_ADS_FREE, true)
    ...
    fun subscription(activity: Activity, itemSKU: String) {
        val productList = mutableListOf( //Product 1
            QueryProductDetailsParams.Product.newBuilder()
                .setProductId(itemSKU)
    ...
    private fun reStart(){
        ...
        val intent = Intent(MyApplication.mInstance, SplashActivity::class.java)
...
// Start the new activity
    ...
    fun oneTimePurchaseDetails(itemSkuId: String, forTrail: Boolean) {
        try{
            val productList = mutableListOf( //Product 1
                QueryProductDetailsParams.Product.newBuilder()
                    .setProductId(itemSkuId)
            ...
            billingClient?.queryProductDetailsAsync(
                ...
                if(prodDetailsList.isNotEmpty()){
                    Log.d(
                        TAG,
                        "oneTimePurchaseDetails: queryProductDetailsAsync size" + prodDetailsList.size
                    )
                    if (forTrail){
                        Constants.oneTimeProductPremiumPriceTrail =
                            prodDetailsList[0].subscriptionOfferDetails?.get(0)?.pricingPhases?.pricingPhaseList?.get(
                                0
                            )?.formattedPrice.toString()
                        Constants.afterDiscountTrail =
                            prodDetailsList[0].subscriptionOfferDetails?.get(0)?.pricingPhases?.pricingPhaseList?.get(
                                1
                            )?.formattedPrice.toString()
                    }else {
                        Constants.oneTimeProductPremiumPrice =
                            prodDetailsList[0].subscriptionOfferDetails?.get(0)?.pricingPhases?.pricingPhaseList?.get(
                                0
                            )?.formattedPrice.toString()
                        Constants.afterDiscount =
                            prodDetailsList[0].subscriptionOfferDetails?.get(0)?.pricingPhases?.pricingPhaseList?.get(
                                1
                            )?.formattedPrice.toString()
                        Log.d(TAG, "price is ${Constants.oneTimeProductPremiumPrice}")
                    }
                }
            }
        }
